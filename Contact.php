
<?php include "header.php" ?>
   <section id="ContactContent">
      <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12">
          <div class="contact_area">
            <h1>Contacts</h1>
            <p>Vestibulum id nisl a neque malesuada hendrerit. Mauris ut porttitor nunc, ut volutpat nisl. Nam ullamcorper ultricies metus vel ornare. Vivamus tincidunt erat in mi accumsan, a sollicitudin risus vestibulum. Nam dignissim purus vitae nisl adipiscing ultricies. Pellentesque in porttitor tellus. Integer fermentum in sem eu tempus. In eu metus vitae nibh laoreet sollicitudin et ac lectus. Curabitur blandit velit elementum augue elementum scelerisque.</p>
            <div class="contact_bottom">
              <div class="contact_us wow fadeInRightBig">
                <h2>Contact Us</h2>
                <form action="DbTransactions.php"  method="POST"  class="contact_form">
                  <input name="Name" class="form-control" type="text" placeholder="Name(required)"  required="" >
                  <input name="Email" class="form-control" type="email" placeholder="E-mail(required)"  required="" >
                  <input name="Subject" class="form-control" type="text" placeholder="Subject" >
                  <textarea name="Message"  class="form-control" placeholder="Message(required)" cols="30" rows="10" required="" ></textarea>
                  <input type="submit" value="Send" name="AddContact">
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
<?php include "footer.php"?>
